...
import OptimizedCharts from "./OptimisedCharts"
...
// Cache the instructor data fetching function
const getInstructorData = unstable_cache(
  async (userId: string) => {
    await connectMongoose()

    const rawCourses = (await Course.find({ userId })
...
export default async function InstructorDashboardPage() {
  ...
  const { courses, enrollments, analytics } = await getInstructorData(session.user.id)
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-md p-6">
            ...
            <p className="text-2xl font-bold text-cyan-600 dark:text-cyan-400">{analytics.totalCourses}</p>
          </div>
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Published Courses</h3>
            <p className="text-2xl font-bold text-green-600 dark:text-green-400">{analytics.publishedCourses}</p>
          </div>
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-md p-6">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Total Enrollments</h3>
            <p className="text-2xl font-bold text-lime-600 dark:text-lime-400">{analytics.totalEnrollments}</p>
          </div>
          <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-md p-6">
        ...
        <div className="mb-8">
          <OptimizedCharts courses={courses} enrollments={enrollments} />
        </div>

        {/* Recent Courses Table */}
        <div className="mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 dark:text-white mb-4">Recent Courses</h2>
        ...
        {/* Recent Enrollments Table - Limited to 5 for performance */}
        <div>
          <h2 className="text-2xl font-semibold text-gray-800 dark:text-white mb-4">Recent Enrollments</h2>
          <div className="overflow-x-auto">
            <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-md min-w-full inline-block align-middle">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                <thead className="bg-gray-50 dark:bg-neutral-700">
